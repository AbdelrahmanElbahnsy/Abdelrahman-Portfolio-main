    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6">
      ...
      <div className="relative w-full max-w-[680px] max-h-[90vh] md:max-h-[85vh] bg-[#0d1321] border border-[#1e293b] rounded-[20px] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-start justify-between p-6 border-b border-[#1e293b] bg-[#0d1321] shrink-0">
          <div className="min-w-0">
            <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-gray-400 mb-1 block">
              {social ? 'EDIT SOCIAL LINK' : 'NEW SOCIAL LINK'}
            </span>
            <span className="text-[11px] font-mono text-[#14f195]/60 mb-2 block">
              {social
                ? `SOCIAL LINK ${String(social.order ?? '').toString().padStart(2, '0')}`
                : `SOCIAL LINK ${String(nextOrderNumber ?? 0).toString().padStart(2, '0')}`
              }
            </span>
            <h2 className="text-xl md:text-2xl font-bold text-white tracking-tight truncate">
              {social ? (
                <span className="truncate">{formData.platform || social.platform}</span>
              ) : (
                formData.platform?.trim() || 'Untitled Social Link'
              )}
            </h2>
          </div>
          <button
        ...
        <div className="flex-grow overflow-y-auto p-6 md:p-8 scrollbar-thin scrollbar-thumb-[#1e293b] scrollbar-track-transparent">
          <form id="social-form" onSubmit={handleSubmit} className="space-y-2">
            ...
            <div className="space-y-5">
              <InputField
                ...
                helper="The exact action URL for the connection."
              />
            </div>
            ...
            <div className="space-y-5">
              ...
              <InputField
                ...
                isMonospace
                placeholder="0"
                helper="Controls display position across the portfolio."
              />
            </div>
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t border-[#1e293b] bg-[#0d1321] shrink-0">
          ...
          <button
            type="submit"
            form="social-form"
            disabled={isSaving || (!isDirty && !!social)}
          ...
          >
            {isSaving ? (
              ...
              <span>{social ? 'Save Changes' : 'Publish Link'}</span>
            )}
          </button>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// SOCIAL CARD (Grid View)
// ─────────────────────────────────────────────────────────────────────────────
const SocialCard = ({ social, onEdit, onDelete, onMoveUp, onMoveDown, isFirst, isLast }) => {
  const hasUrl = social.url && String(social.url).trim().length > 0;
  const orderDisplay = social.order !== undefined && social.order !== null && social.order !== ''
    ? String(social.order).padStart(2, '0')
    : '—';
  ...
  return (
    <div className="bg-[#0f172a] rounded-[16px] border border-[#1e293b] p-6 shadow-sm hover:shadow-xl hover:border-[#334155] transition-all duration-300 flex flex-col h-full group relative overflow-hidden hover:-translate-y-0.5">
      ...
      {/* Top Header: Order & Icon */}
      <div className="mb-4 flex items-center justify-between">
        <span className="text-[11px] font-mono font-bold uppercase tracking-[0.2em] text-[#14f195]/80">
          {orderDisplay} / LINK
        </span>
        {social.icon && (
          <div className="w-8 h-8 rounded-full bg-[#1e293b]/50 border border-[#1e293b] flex items-center justify-center text-gray-400 group-hover:text-white transition-colors">
      ...
      <div className="flex-grow flex flex-col">
        <div className="mb-4">
          <h3 className="text-lg font-bold text-white tracking-tight leading-snug break-words">
            {social.platform || 'Untitled Link'}
          </h3>
        </div>

        {/* Technical Icon class indicator */}
        {social.icon && (
          <div className="mb-4">
            <span className="text-[10px] font-mono text-gray-600 bg-[#1e293b]/50 px-2 py-1 rounded border border-[#1e293b] truncate inline-block max-w-full">
              {social.icon}
            </span>
          </div>
        )}
...
const SocialsManager = () => {
  const { data: items, loading, create, update, remove, subscribe } = useFirestoreCrud('socials', {
    orderByField: 'order',
    orderDirection: 'asc'
  });
  ...
  const handleEdit = useCallback((social) => {
    setEditingSocial(social);
    setIsEditorOpen(true);
  }, []);

  const handleDeleteRequest = useCallback((social) => {
  ...
  const handleSave = useCallback(async (formData) => {
    ...
    try {
      ...
      if (editingSocial) {
        await update(editingSocial.id, payload);
  ...
  }, [editingSocial, items, update, create]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(social => {
      return (
        (social.platform || '').toLowerCase().includes(query) ||
        (social.url || '').toLowerCase().includes(query)
      );
    });
  }, [items, searchQuery]);
  ...
  const metrics = useMemo(() => {
    if (!items) return { total: 0, active: 0, platforms: 0 };
    const active = items.filter(s => s.url && s.url.trim() !== '').length;
    const uniquePlatforms = new Set(items.map(s => (s.platform || '').toLowerCase().trim())).size;
    return {
      total: items.length,
      active,
    ...
    <div className="max-w-[1600px] mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div>
          ...
          <p className="text-gray-400 max-w-xl leading-relaxed text-sm">
            Manage the external platforms and contact channels displayed across the public portfolio.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#0d1321] border border-[#1e293b] rounded-2xl p-5">
          <span className="text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest block mb-2">Total Connections</span>
          <span className="text-2xl font-bold text-white">{metrics.total.toString().padStart(2, '0')}</span>
        </div>
        <div className="bg-[#0d1321] border border-[#1e293b] rounded-2xl p-5">
          <span className="text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest block mb-2">Active Links</span>
          <span className="text-2xl font-bold text-[#14f195]">{metrics.active.toString().padStart(2, '0')}</span>
        </div>
        <div className="bg-[#0d1321] border border-[#1e293b] rounded-2xl p-5">
          <span className="text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest block mb-2">Unique Platforms</span>
          <span className="text-2xl font-bold text-blue-400">{metrics.platforms.toString().padStart(2, '0')}</span>
        </div>
      </div>
      ...
      {!items || items.length === 0 ? (
        <div className="border border-dashed border-[#1e293b] rounded-2xl p-12 text-center flex flex-col items-center justify-center bg-[#0d1321]/50 min-h-[300px]">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Social Connections Empty</h3>
          <p className="text-gray-400 mb-6 max-w-sm">No social links are currently configured. Add your first connection to show on the portfolio.</p>
          <button
            onClick={handleAddNew}
        ...
        <div className="border border-[#1e293b] rounded-2xl p-12 text-center bg-[#0d1321]">
          ...
          <h3 className="text-lg font-bold text-white mb-2">No Social Links Found</h3>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[#1e293b] bg-[#0f172a]">
                  ...
                  <th className="p-4 text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest">Platform</th>
                  <th className="p-4 text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest hidden md:table-cell">URL</th>
                  <th className="p-4 text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1e293b]">
                {filteredItems.map((social, idx) => (
                  <tr key={social.id} className="hover:bg-[#1e293b]/30 transition-colors group">
                    <td className="p-4 text-center">
                      <span className="text-xs font-mono text-gray-500">
                        {social.order !== undefined && social.order !== null && social.order !== '' 
                          ? String(social.order).padStart(2, '0') 
                          : '—'}
                      </span>
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        {social.icon && (
                          <div className="w-8 h-8 rounded-lg bg-[#1e293b] flex items-center justify-center text-gray-400 shrink-0">
                            <i className={`${social.icon} text-xs`}></i>
                          </div>
                        )}
                        <span className="font-bold text-white whitespace-nowrap">{social.platform || 'Untitled'}</span>
                      </div>
                    </td>
                      ...
                      <div className="max-w-[200px] lg:max-w-[300px] truncate">
                        {social.url ? (
                          <a 
                            ...
                            target="_blank" 
                            rel="noopener noreferrer" 
                            className="text-xs font-mono text-gray-400 hover:text-[#14f195] transition-colors truncate"
                          >
                            {social.url}
                          </a>
                        ) : (
                          <span className="text-xs text-gray-600 italic">No URL</span>
                        )}
                      </div>
                    </td>
