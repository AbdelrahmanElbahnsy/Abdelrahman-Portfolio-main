...
// PREVIEW MODAL
// ─────────────────────────────────────────────────────────────────────────────
const PreviewModal = ({ item, onClose, isSelectionMode, onSelectAsset }) => {
  if (!item) return null;
  ...
  return (
    <div className="fixed inset-0 z-[70] flex justify-center items-center" aria-modal="true" role="dialog">
      ...
      <div className="relative w-full max-w-[900px] h-full max-h-[90vh] flex flex-col md:flex-row bg-[#050914] border border-[#1e2d42] shadow-2xl rounded-2xl overflow-hidden m-4 animate-in zoom-in-95 duration-200">
        ...
        <div className="w-full md:w-[350px] flex flex-col bg-[#050914] overflow-y-auto">
          ...
          <div className="p-6 space-y-6 flex-grow flex flex-col">
            ...
            {isSelectionMode && onSelectAsset && (
              <div className="mt-6 pt-6 border-t border-[#1e2d42]">
                <button
                  onClick={() => onSelectAsset(item.url)}
                  className="w-full py-3 bg-[#14f195] hover:bg-[#10d482] text-[#090e1a] rounded-xl font-bold transition-colors shadow-[0_0_20px_rgba(20,241,149,0.15)] flex items-center justify-center gap-2"
                >
                  <ImageIcon className="w-4 h-4" /> Select This Asset
                </button>
              </div>
            )}
          </div>
...
// CORE COMPONENT
// ─────────────────────────────────────────────────────────────────────────────
export default function MediaLibraryCore({ isSelectionMode = false, onSelectAsset }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (isSelectionMode && onSelectAsset && newUrl) {
        onSelectAsset(newUrl);
      }
    } catch (err) {
  ...
  return (
    <div className="w-full">
      ...
      {!isSelectionMode ? (
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 bg-[#0b1120] p-4 rounded-xl border border-[#1a2440]">
           <div className="flex items-center gap-4">
             <div className="flex flex-col">
               <span className="text-[10px] font-mono text-[#14f195] uppercase tracking-widest font-bold mb-0.5">Library Index</span>
               <span className="text-white font-bold text-sm leading-none">{stats.total} Assets</span>
             </div>
           </div>
           ...
           <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button
              onClick={() => fileInputRef.current?.click()}
              disabled={isUploading}
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-[#14f195] text-[#090e1a] text-sm font-bold rounded-lg hover:bg-[#10d482] transition-all shadow-[0_0_15px_rgba(20,241,149,0.1)] disabled:opacity-50 shrink-0"
            >
              {isUploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <UploadCloud className="w-4 h-4" />}
              {isUploading ? `Uploading ${uploadProgress}%` : 'Upload New Asset'}
            </button>
          </div>
        </div>
      )}
            ...
            {filteredMedia.map((item) => (
              <div key={item.id} className="bg-[#0b1120] border border-[#1a2440] rounded-xl overflow-hidden hover:border-[#334155] transition-all group flex flex-col shadow-sm hover:shadow-xl">
                ...
                <div 
                  ...
                  onClick={() => isSelectionMode && onSelectAsset ? onSelectAsset(item.url) : setPreviewTarget(item)}
                >
                  <img 
                    src={item.url} 
                  ...
                  {/* Hover Overlay */}
                  <div className="absolute inset-0 bg-[#030712]/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center backdrop-blur-[2px] gap-2">
                    {isSelectionMode && onSelectAsset && (
                      <button
                        onClick={(e) => { e.stopPropagation(); onSelectAsset(item.url); }}
                        className="bg-[#14f195] text-[#090e1a] px-4 py-2 rounded-lg text-xs font-bold shadow-lg hover:bg-[#10d482] transition-colors w-2/3"
                      >
                        Select Asset
                      </button>
                    )}
                    <button
                       ...
                       className={`${isSelectionMode ? 'bg-[#1e2d42] text-white hover:bg-[#2a3a52]' : 'bg-[#14f195] text-[#090e1a]'} px-4 py-2 rounded-lg text-xs font-bold shadow-lg flex items-center justify-center gap-2 transition-colors ${isSelectionMode ? 'w-2/3' : 'w-auto'}`}
                    >
                      ...
                      <td className="px-5 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {isSelectionMode && onSelectAsset && (
                            <button
                              onClick={() => onSelectAsset(item.url)}
                              className="px-3 py-1.5 rounded bg-[#14f195] hover:bg-[#10d482] text-[#090e1a] text-xs font-bold transition-colors"
                            >
                              Select
                            </button>
                          )}
